	...
	public:
		...
		caret_gatherer(
			...
			while (first != _carets.end() && _queued.size() < maximum_num_lookahead_carets) {
				std::pair<std::size_t, std::size_t> range = std::minmax(first->first.first, first->first.second);
		...
		void skip_line(bool stall, std::size_t posafter) {
			...
			for (auto it = _queued.begin(); it != _queued.end(); ) {
				std::pair<std::size_t, std::size_t> range = std::minmax((*it)->first.first, (*it)->first.second);
				if ( // too early
