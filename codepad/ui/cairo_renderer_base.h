...
namespace codepad::ui::cairo {
	...
	public:
		...
		void set_font_family(str_view_t, std::size_t, std::size_t) override {
			// TODO
		}
	...
	public:
		...
		std::unique_ptr<ui::formatted_text> create_formatted_text(
			str_view_t text, const font_parameters &font, colord c, vec2d size, wrapping_mode wrap,
		...
		std::unique_ptr<ui::formatted_text> create_formatted_text(
			...
			return _create_formatted_text_impl(
				str_view_t(reinterpret_cast<const char*>(text.c_str()), text.size()),
		...
		///
		std::unique_ptr<ui::plain_text> create_plain_text(str_view_t, ui::font&, double) override {
			// TODO
