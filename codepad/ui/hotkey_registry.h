...
namespace codepad {
	...
	namespace ui {
		...
		namespace gestures {
			...
			inline std::pair<modifier_keys, str_view_t> split(str_view_t text) {
				...
				for (auto last = cur; cur != text.end(); last = cur) {
					if (codepoint cp; encodings::utf8::next_codepoint(cur, text.end(), cp)) {
						if (last != start && cp == delimiter) {
							str_view_t comp_str(&*start, last - start);
				...
				return {mod, str_view_t(&*start, text.end() - start)};
			}
		}
		...
		struct key_gesture {
			...
			inline static std::optional<key_gesture> parse(str_view_t text) {
		...
		struct mouse_gesture {
			...
			inline static std::optional<mouse_gesture> parse(str_view_t text) {
				auto [mod, primary] = gestures::split(text);
