...
namespace codepad::ui::tabs {
	...
	public:
		...
		/// Creates a new \ref tab in a \ref host in the last focused \ref os::window_base. If there are no
		...
		void update_drag() {
			if (_drag) {
				...
				vec2d minpos;
				double minsql = 0.0;
				if (_dtype != drag_destination_type::combine_in_tab) { // find best host to target
				...
				if (_dtype != drag_destination_type::combine_in_tab) { // check nearest host
					...
					_drag_tab_window->set_position(mouse - _dragdiff.convert<int>());
				}
		...
		void start_drag_tab(tab &t, vec2d diff, rectd layout, std::function<bool()> stop = []() {
			...
			_drag_tab_window->set_client_size(t.get_button().get_layout().size().convert<int>());
	...
	protected:
		...
		host *_dest = nullptr; ///< The destination \ref host of the \ref tab that's currently being dragged.
