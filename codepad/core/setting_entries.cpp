...
namespace codepad {
	...
	public:
		...
		setting(std::vector<str_t> k, parser_type p) :
	...
	namespace editors {
		...
		settings::retriever_parser<str_t> &editor::get_font_family_setting(settings &set) {
			static setting<str_t> _setting(
				{ u8"editor", u8"font_family" },
				settings::basic_parsers::basic_type_with_default<str_t>("Courier New")
			);
			return _setting.get(set);
		}

		settings::retriever_parser<vector<str_t>> &editor::get_interaction_modes_setting(settings &set) {
			static setting<vector<str_t>> _setting(
		...
		namespace code {
			settings::retriever_parser<std::vector<str_t>> &contents_region::get_backup_fonts_setting(settings &set) {
				static setting<vector<str_t>> _setting(
					{ u8"editor", u8"backup_fonts" },
					settings::basic_parsers::basic_type_with_default<std::vector<str_t>>(
						std::vector<str_t>(), json::array_parser<str_t>()
						)
				);
				return _setting.get(set);
			}
