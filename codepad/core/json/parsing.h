	...
	namespace parsing {
		...
		namespace _details {
			...
			template <typename ValueType> std::shared_ptr<context_node<ValueType>> spawn_child(
				std::shared_ptr<context_node<ValueType>> parent, str_t id
			) {
				return std::make_shared<context_node<ValueType>>(
					typename context_node<ValueType>::identifier_t(std::in_place_type<str_t>, std::move(id)),
					std::move(parent)
					);
			}
			/// Creates a child node from the given identifier.
		...
		public:
			...
			public:
				...
				value_t<ValueType> value() const {
					return value_t<ValueType>(_it.value(), _details::spawn_child(_obj->_node, str_t(_it.name())));
				}
