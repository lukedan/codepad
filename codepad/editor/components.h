	...
	namespace editor {
		...
		public:
			vec2d get_desired_size() const override {
				size_t ln = _get_editor<codebox_editor_code>()->get_context()->num_lines(), w = 0;
				for (; ln > 0; ++w, ln /= 10) {
				}
				vec2d res = get_padding().size();
				res.x += static_cast<double>(w) * codebox_editor_code::get_font().maximum_width();
				return res;
			}
		protected:
			void _render() const override {
				codebox_editor_code *editor = _get_editor<codebox_editor_code>();
				...
				for (size_t i = line_beg + 1; i <= line_end; ++i, cury += lh) {
					...
					double w = ui::text_renderer::measure_plain_text(curlbl, *codebox_editor_code::get_font().normal).x;
					ui::text_renderer::render_plain_text(
						curlbl, *codebox_editor_code::get_font().normal,
