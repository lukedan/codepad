...
namespace codepad::os {
	...
	inline int _interpret_open_mode(open_mode mode) {
		int openflags =
			(test_bit_any(mode, open_mode::create) ? O_CREAT : 0) |
			(test_bit_any(mode, open_mode::open_and_truncate) ? O_TRUNC : 0);
	...
	void file_mapping::_map_impl(const file &f, access_rights acc) {
		...
		_ptr = mmap(
			nullptr, _len,
			(test_bit_any(acc, access_rights::read) ? PROT_READ : 0) |
			(test_bit_any(acc, access_rights::write) ? PROT_WRITE : 0),
