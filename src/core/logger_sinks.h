...
namespace codepad::logger_sinks {
	...
	protected:
		...
		void _print_left(color_scheme::entry scheme, std::u8string_view text = u8" ") {
			_color(scheme);
			std::cout <<
		...
		void _print_w(std::u8string_view msg, color_scheme::entry scheme, color_scheme::entry banner, std::size_t w) {
			_color(scheme);
			std::size_t cl = 0;
	...
	protected:
		...
		inline static std::string_view _get_level_label(log_level l) {
			switch (l) {
			case log_level::debug:
				return "D";
			case log_level::info:
