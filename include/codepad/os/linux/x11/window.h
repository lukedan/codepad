				...
				case MotionNotify:
					{
						...
						cursor c = get_current_display_cursor();
				...
				case KeyPress:
					{
						...
						if (!hotkey_manager.on_key_down(gest)) {
							...
							if (stat == XBufferOverflow) {
								...
								_onevent(&window::_on_keyboard_text, convert_to_default_encoding(lbuf, lbuf + sz));
								std::free(lbuf);
							} else if (stat != XLookupKeySym) {
								if (ksym != XK_BackSpace && ksym != XK_Delete) {
									_onevent(&window::_on_keyboard_text, convert_to_default_encoding(buffer, buffer + sz));
								}
							}
						}
					}
