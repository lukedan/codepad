...
namespace codepad::os {
	...
	protected:
		...
		inline static gboolean _on_draw_event(GtkWidget*, cairo_t *cr, window *wnd) {
		...
		ui::_details::gtk_object_ref<cairo_surface_t> _create_similar_surface(
			ui::window_base &wnd, int width, int height
		) override {
			return ui::_details::make_gtk_object_ref_give(gdk_window_create_similar_surface(
				gtk_widget_get_window(_details::cast_window(wnd).get_native_handle()),
		...
		void _new_window(ui::window_base &wnd) override {
			...
			g_signal_connect(
				_details::cast_window(wnd).get_native_handle(), "draw",
