...
namespace codepad::ui::tabs {
	...
	protected:
		...
		struct _child_data {
			...
			/// Similar to \ref set_offset(), but used when we're certain that the tab is being dragged.
			void set_dragging_offset(animated_tab_buttons_panel&, element&, double offset);
		...
		info_event<tab_manager::drag_ended_info>::token _droptok; ///< Used to handle \ref tab_manager::drag_ended.
		/// Used to handle \ref tab_manager::drag_move_tab_button.
		info_event<tab_manager::drag_update_info>::token _updatetok;
		...
		void _on_child_added(element &elem, element *before) override {
			...
			if (auto *btn = dynamic_cast<tab_button*>(&elem)) { // initialize info
				...
				// TODO this is not good - but the alternative is to have a specialized panel type which is not good
				//      either (in which case we might as well just make all events virtual functions), or to have
				//      the manager notify ALL hosts when events occur
				if (man.is_dragging_any_tab() && &man.get_active_dragging_tab().target->get_button() == &elem) {
		...
		void _on_start_drag() {
			tab_manager &man = _get_tab_manager();
			_droptok = man.drag_ended += [this](tab_manager::drag_ended_info &info) {
				// animation
				for (auto &t : info.dragging_tabs) {
					tabs::tab_button &button = t.target->get_button();
					auto *data = _get_data(button);
					data->set_offset(*this, button, data->current_offset);
				}
				_on_end_drag();
			};
			_updatetok = man.drag_move_tab_buttons += [this](tab_manager::drag_update_info &info) {
				_on_drag_update(info);
			};
		}
		/// Called when the user stops dragging a tab or when the tab is dragged away from this panel. This function
		/// unregisters handlers for drag events.
		void _on_end_drag() {
			// unregister events
			tab_manager &man = _get_tab_manager();
			man.drag_ended -= _droptok;
			man.drag_move_tab_buttons -= _updatetok;
		}
		/// Called when \ref tab_manager::drag_move_tab_button is invoked.
		void _on_drag_update(tab_manager::drag_update_info&);
