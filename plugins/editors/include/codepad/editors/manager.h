...
namespace codepad::editors {
	...
	public:
		...
		bool unregister_renderer(const std::u8string &name) {
			return _mapping.erase(name) != 0;
		}

		/// Finds the creation function for decoration renderers with the given type.
		[[nodiscard]] const renderer_type_info *find_renderer_type(const std::u8string &name) const {
		...
		/// registered.
		[[nodiscard]] std::shared_ptr<decoration_renderer> create_renderer(const std::u8string &name) const {
	...
	protected:
		...
		std::unordered_map<std::u8string, renderer_type_info> _mapping;
	};
